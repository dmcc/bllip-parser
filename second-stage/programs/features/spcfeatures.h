...
// spcfeatures.h
//
// Mark Johnson, forked from spefeatures.h on 14th November, 2005
//
// A spcfeature has extra-sentential context information
// 
#ifndef SPCFEATURES_H
#define SPCFEATURES_H
...
public:
  ...
  typedef std::vector<int> Is;     //!< Is is a vector of int
  typedef std::vector<symbol> Ss;  //!< Ss is a vector of symbols
  ...
  inline static symbol rootmarker() { static symbol r("*ROOT*"); return r; }
  ...
  //! is_bounding_node() is true of nodes labeled ROOT, S or SBAR
  //
  ...
  //! in_root_clause() is true of nodes that are in the matrix or root clause.
  //
  inline static bool in_root_clause(const sptree* node) {
    while (!is_bounding_node(node) && node->label.parent) // up to bounding node
      node = node->label.parent;
    while (is_bounding_node(node) && node->label.parent) // over bounding nodes
      node = node->label.parent;
    if (node->label.parent == NULL)
      return true;
    else
      return false;
  }  // FeatureClass::in_root_clause()
...
//! context_type{} stores information from the previous sentences
//! that can be conditioned on by following sentences.
//
struct context_type {

  typedef std::vector<symbol> Ss;
  std::string labelstem;      //!< stem from sentence label
  int labelnumber;            //!< number from sentence label
  symbol stype;               //!< type of sentence
  symbol last_stype;          //!< type of preceding sentence

  void update(const sp_sentence_type& s) {

    static symbol INIT("*INIT*");
    static symbol NONE("*NONE*");
    static symbol DQUOTE("''");
    static symbol FULLSTOP(".");

    assert(s.nparses() > 0);
    std::string newlabelstem;
    int newlabelnumber = 0;
    std::string::size_type pos =  s.label.rfind('.');
    if (pos != std::string::npos) {
      newlabelstem.assign(s.label, 0, pos);
      newlabelnumber = lexical_cast<int>(s.label.substr(pos+1));
    }
    else if (s.label.size() > 0)
      newlabelnumber = lexical_cast<int>(s.label);
    if (newlabelstem == labelstem && newlabelnumber == labelnumber+1) 
      last_stype = stype;
    else
      last_stype = INIT;

    std::vector<const sptree*> preterms;
    s.parses[0].parse->preterminal_nodes(preterms);
    size_type npreterms = preterms.size();
    assert(npreterms > 0);
    if (preterms[npreterms-1]->label.cat == FULLSTOP)
      stype = preterms[npreterms-1]->child->label.cat;
    else if (npreterms > 1 && preterms[npreterms-1]->label.cat == DQUOTE
	     && preterms[npreterms-2]->label.cat == FULLSTOP)
      stype = preterms[npreterms-2]->child->label.cat;
    else 
      stype = NONE;

    labelstem = newlabelstem;
...
//! sentence_context is a global variable holding information from
//! the preceding context.  Yes, I know it would be better coding style
//! not to use a global variable, and to thread this variable though
//! the functions that use it.  But ugh, what a nuisance!
//
...
private:
  struct extract_features_visitor {
    ...
    void operator() (const sp_sentence_type& sentence) {

      assert(sentence.nparses() > 0);

      if (debug_level > 1000)
	std::cerr << '\n' << sentence.parses[0].parse << '\n' << std::endl;

      sentence_context.update(sentence);

      foreach (FeatureClassPtrs, it, fcps)
	(*it)->extract_features(sentence);
...
public:
  ...
  inline void features_050904();
  inline void features_context0();
...
public:
  ...
  enum root_level { noroot, rootonly, roottype, lastroottype };
  ...
  RuleFeatureClass(std::string identifier_stem,   //!< Stem of identifier
		   ...
		   root_level label_root,	  //!< Annotate with "in root context"
  ...
  const root_level label_root;
  const bool label_conjunct;
  const annotation_level head;         //!< annotation on rule's head
  const annotation_level functional;   //!< annotation on (projections of) functional categories
  const annotation_level all;          //!< annotation on all words
  const annotation_type type;          //!< syntactic or semantic annotation
  ...
  void push_ancestor_features(const sptree* node, Feature& f) {
    ...
    if (in_root_clause(node)) {
      if (label_root == rootonly)
	f.push_back(rootmarker());
      else if (label_root == roottype)
	f.push_back(sentence_context.stype);
    }

    if (label_root == lastroottype)
      f.push_back(sentence_context.last_stype);
...
public:
  ...
  Rule(size_type nanctrees = 0,         //!< Number of ancestor local trees
       ...
       root_level label_root = noroot,  //!< Annotate with "in root context"
...
public:
  ...
  size_type nleftprec, nleftsucc, nrightprec, nrightsucc;  // number of POS surrounding edges
  std::string identifier_string;  // will hold its identifier
...
inline void FeatureClassPtrs::features_connll() {
  ...
  push_back(new Rule(0, 0, Rule::noroot));
  push_back(new Rule(0, 0, Rule::noroot, true));
  push_back(new Rule(0, 0, Rule::noroot, false, Rule::lexical));
  push_back(new Rule(0, 0, Rule::noroot, false, Rule::none, Rule::lexical));
  push_back(new Rule(0, 0, Rule::noroot, false, Rule::lexical, Rule::lexical));

  push_back(new NGram(1, 1, NGram::noroot, true));
  push_back(new NGram(2, 1, NGram::rootonly, true));
  push_back(new NGram(3, 1, NGram::rootonly, true));
  push_back(new NGram(2, 1, NGram::noroot, false, NGram::lexical));
  push_back(new NGram(2, 1, NGram::noroot, false, NGram::none, NGram::lexical));
...
inline void FeatureClassPtrs::features_050902() {
  ...
  push_back(new Rule());
  push_back(new Rule(0, 1));
  push_back(new Rule(0, 0, Rule::rootonly));
  push_back(new Rule(0, 0, Rule::noroot, true));
  push_back(new Rule(0, 0, Rule::noroot, false, Rule::lexical));
  push_back(new Rule(0, 0, Rule::noroot, false, Rule::none, Rule::lexical));
  push_back(new Rule(0, 0, Rule::noroot, false, Rule::lexical, Rule::lexical));
...
inline void FeatureClassPtrs::features_050904() {
  push_back(new NLogP());
  ...
  for (size_type binflag = 0; binflag < 2; ++binflag)
	...
	for (size_type nrightprec = 0; nrightprec <= maxwidth; ++nrightprec)
	  for (size_type nrightsucc = 0; nrightsucc <= maxwidth; ++nrightsucc)
	    if (nleftprec + nleftsucc + nrightprec + nrightsucc <= maxsumwidth)
	      push_back(new Edges(binflag, nleftprec, nleftsucc, nrightprec, nrightsucc));
}  // FeatureClassPtrs::features_050904()


inline void FeatureClassPtrs::features_context0() {
  push_back(new NLogP());
  push_back(new Rule(0, 0, Rule::lastroottype));
}  // FeatureClassPtrs::features_context0()
