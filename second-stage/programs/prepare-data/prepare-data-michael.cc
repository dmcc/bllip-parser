...
// prepare-data-michael.cc -- converts Michael Collins' files into my training format
//
// Mark Johnson, 14th September 2003
//
// This version does not reorder trees in any way, but keeps them in the same
// order so Michael can merge them with his own features.
//
// It was produced mainly by chopping bits out of prepare-data.cc

const char usage[] =
"Usage: prepare-data-michael penn-wsj-regex nsentences collins-tree-command collins-score-command\n"
"\n"
"where:\n"
...
struct collins_info {
  ...
  std::string parse;
  collins_info(double logprob, double precrec, double condprob, double n_gold_constituents,
	       const std::string& parse) 
    : logprob(logprob), precrec(precrec), condprob(condprob), 
      n_gold_constituents(n_gold_constituents), parse(parse) { }
};

typedef std::vector<collins_info> CIs;
...
void write_collins_trees(const penn& p, CIs& parsestats, FILE* of, 
			 precrec_type& pr, bool writeflag = false)
{
  assert(!parsestats.empty());
  // look up yield in p
  Ss yield;
  tree* tp = readtree_root(parsestats[0].parse.c_str());
  tree_terminals(tp, yield);                     // this yield has no punctuation
  STIrange r = p.yield_trees.equal_range(yield); // r is the set of gold trees
  yield.clear();
  tp->terminals(yield);                          // now yield has punctuation
  std::string finalpunct, modified_treestring;
  if (r.first == r.second)
    std::cerr << "## Can't find yield " << yield 
	      << "\n## best_parse = " << tp
	      << "\n## string = " << parsestats[0].parse << std::endl;
  else {
    ...
    precrec_type::tree_nontermedges(tp, parse_edges);
    ...
    cforeach (CIs, it, parsestats) {
      modified_treestring = it->parse;
		      ...
		      const char score_command[], FILE* of)
{
  ...
  while(true) {
    ...
    if (nread_parse == EOF) {
      ...
      ++nsentences;
      write_collins_trees(p, parsestats, of, pr, true);
    ...
    if (old_parse_id == parse_id) 
      ...
      if (old_parse_id != -1) {
	++nsentences;
	write_collins_trees(p, parsestats, of, pr, nsentences==1);
	parsestats.clear();
      }
	...
	cparse[i] = '\0';  // replace final space with '\0'
      ...
      parsestats.push_back(collins_info(logprob, normscore, condprob, 
					normscore > 0 ? score/normscore : -1,
					parse));
    }
  }
  pclose(tp);
